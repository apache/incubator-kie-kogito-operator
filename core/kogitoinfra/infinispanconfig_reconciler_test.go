...
func TestInfinispanConfigReconciler(t *testing.T) {
	...
	assert.True(t, len(configMap.Data["quarkus.infinispan-client.trust-store-type"]) > 0)
	assert.True(t, len(configMap.Data["quarkus.infinispan-client.trust-store"]) > 0)
	assert.True(t, len(configMap.Data["quarkus.infinispan-client.trust-store-password"]) > 0)
}
