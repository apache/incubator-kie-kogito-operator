...
func Test_CheckKogitoRuntimeExists_exists(t *testing.T) {
	...
	ns := t.Name()
	kubeCli := test.NewFakeClientBuilder().UseScheme(internal.GetRegisteredSchema()).AddK8sObjects(
...
func Test_CheckKogitoRuntimeExists_notExists(t *testing.T) {
	runtimeServiceName := "runtime-service"
	ns := t.Name()
	kubeCli := test.NewFakeClientBuilder().UseScheme(internal.GetRegisteredSchema()).AddK8sObjects(&v1.Namespace{ObjectMeta: v12.ObjectMeta{Name: ns}}).Build()
	resourceCheckService := NewResourceCheckService()
...
func Test_CheckKogitoBuildExists_exists(t *testing.T) {
	...
	ns := t.Name()
	kubeCli := test.NewFakeClientBuilder().UseScheme(internal.GetRegisteredSchema()).AddK8sObjects(&v1.Namespace{ObjectMeta: v12.ObjectMeta{Name: ns}},
		&v1beta1.KogitoBuild{
			ObjectMeta: v12.ObjectMeta{
