...
func AddRuntimeFlags(command *cobra.Command, flags *RuntimeFlags) {
	...
	command.Flags().BoolVar(&flags.EnableMonitoring, "enable-monitoring", false, "If set to true, deployed Kogito service will support integration with prometheus+grafana for the monitoring. Default to false")
