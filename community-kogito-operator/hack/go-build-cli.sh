...
if [ "$release" = "true" ]; then
  ...
  do
    ...
    packTemplateFiles
    CGO_ENABLED=0 GOOS="${i}" GOARCH="${arch}" go build -v -a -o build/_output/bin/kogito github.com/kiegroup/kogito-cloud-operator/community-kogito-operator/cmd/kogito
    if [ $? -ne 0 ]; then
  ...
  packTemplateFiles
  CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -v -a -o build/_output/bin/kogito github.com/kiegroup/kogito-cloud-operator/community-kogito-operator/cmd/kogito
  cleanTemplateFiles
fi
