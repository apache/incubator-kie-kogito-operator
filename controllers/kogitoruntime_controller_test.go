...
func TestReconcileKogitoRuntime_Reconcile(t *testing.T) {
	...
	assert.Equal(t, deployment.Spec.Template.Spec.Containers[0].Lifecycle.PostStart.Exec.Command, kogitoruntime.PodStartExecCommand)

	configMap := &corev1.ConfigMap{ObjectMeta: v1.ObjectMeta{Name: kogitoruntime.GetProtoBufConfigMapName(instance.Name), Namespace: instance.Namespace}}
	exists, err = kubernetes.ResourceC(cli).Fetch(configMap)
	assert.NoError(t, err)
	assert.True(t, exists)
	assert.Equal(t, kogitoruntime.GetProtoBufConfigMapName(instance.Name), configMap.Name)
}
