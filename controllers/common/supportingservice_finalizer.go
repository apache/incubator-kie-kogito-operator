...
// FinalizeKogitoSupportingServiceReconciler reconciles a KogitoSupportingService object
type FinalizeKogitoSupportingServiceReconciler struct {
...
func (f *FinalizeKogitoSupportingServiceReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	b := ctrl.NewControllerManagedBy(mgr).For(f.ReconcilingObject, builder.WithPredicates(pred))
...
func (f *FinalizeKogitoSupportingServiceReconciler) Reconcile(ctx context.Context, request reconcile.Request) (result reconcile.Result, err error) {
	...
	supportingServiceHandler := f.SupportingServiceHandler(kogitoContext)
	...
	infraHandler := f.InfraHandler(kogitoContext)
