...
func TestReconcileKogitoBuildSimple(t *testing.T) {
	...
	r := KogitoBuildReconciler{Client: cli, Scheme: meta.GetRegisteredSchema(), Log: test_log}
	...
	assert.Equal(t, kogitobuild.KogitoQuarkusJVMUbi8Image, kogitoISList.Items[0].Name)
	assert.Equal(t, kogitobuild.KogitoQuarkusUbi8s2iImage, kogitoISList.Items[1].Name)
