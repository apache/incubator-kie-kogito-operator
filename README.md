...
Kogito Operator installs Infinispan and inject the connection properties as environment variables and [application properties via ConfigMap](#kogito-runtimes-properties-configuration) into the service. 
Depending on the runtime, these variables will differ. See the table below:


|Quarkus Runtime                          |Springboot Runtime              |Type                     | Description                                       |Example                |
|-----------------------------------------|--------------------------------|-------------------------|---------------------------------------------------|-----------------------| 
|quarkus.infinispan-client.server-list    |infinispan.remote.server-list   |Application Property     |Service URI from deployed Infinispan               |kogito-infinispan:11222|
|quarkus.infinispan-client.sasl-mechanism |infinispan.remote.sasl-mechanism|Application Property     |Default to `PLAIN`                                 |`PLAIN`                |
|quarkus.infinispan-client.use-auth       |infinispan.remote.use-auth      |Application Property     |Default to `true`                                  |`true`                 |
|QUARKUS_INFINISPAN_CLIENT_AUTH_USERNAME  |INFINISPAN_REMOTE_AUTH_USER_NAME|Environment Variable     |Default username generated by Infinispan Operator  |developer              |
|QUARKUS_INFINISPAN_CLIENT_AUTH_PASSWORD  |INFINISPAN_REMOTE_AUTH_PASSWORD |Environment Variable     |Random password generated by Infinispan Operator   |Z1Nz34JpuVdzMQKi       |

Just make sure that your Kogito Service can read these properties in runtime. 

_**Note:** The Infinispan application properties are derived from the customization described in the Kogito CLI or the `KogitoApp` custom resource. Any manual changes to the properties in the ConfigMap will be overridden._
...
These properties are replaced by the environment variables or [application properties](#kogito-runtimes-properties-configuration) in runtime.
...
value of Kafka server location application property as well. For example, by running:
