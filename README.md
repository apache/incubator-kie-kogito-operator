...
We aim to avoid having a huge monolith inside the reconcile loop that does it all. With that in mind, we separated the responsibility of making the Kubernetes and OpenShift API calls to a package that we call [`inventory`](pkg/controller/kogitoapp/inventory) and the definition of the resources (our domain) into the [`definitions`](pkg/controller/kogitoapp/definitions) package.
...
`Controller` will orchestrate all operations through inventory calls by using its domain model (`Type`). `Inventory` will look for resources `definitions` once its needs to create new resources into the cluster.

### Inventory

In this package we handle all Kubernetes/OpenShift API calls, transforming those operations into meaningful functions that can be used across all controller operations. Take for example the `CreateResourceIfNotExists` function:
...
The `definitions` package defines the structure and dependencies of every resource according to the controller requirements. The following diagram illustrates the relationship between the OpenShift resources for deploying a Kogito Application through the `KogitoApp` controller:

![Kogito App Resources Composition](docs/img/kogitoapp_resource_composition.png?raw=true)

The `definitions` package ensures that each object is created accordingly. Take "for example" the `NewRoleBinding` definition:
