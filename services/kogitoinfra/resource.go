...
import (
	...
	"github.com/kiegroup/kogito-cloud-operator/services/kogitoinfra/infinispan"
	"github.com/kiegroup/kogito-cloud-operator/services/kogitoinfra/kafka"
	"github.com/kiegroup/kogito-cloud-operator/services/kogitoinfra/keycloak"

	"reflect"
)

// getDeployedResources fetches for deployed resources managed by KogitoInfra controller
func GetDeployedResources(client *client.Client, instance *v1alpha1.KogitoInfra) (resources map[reflect.Type][]resource.KubernetesResource, err error) {
	resourcesInfinispan, err := infinispan.GetDeployedResources(instance, client)
	if err != nil {
		return
	}
	resourcesKafka, err := kafka.GetDeployedResources(instance, client)
	if err != nil {
		return
	}
	resourcesKeycloak, err := keycloak.GetDeployedResources(instance, client)
	if err != nil {
		return
	}
...
func CreateRequiredResources(client *client.Client, instance *v1alpha1.KogitoInfra) (resources map[reflect.Type][]resource.KubernetesResource, err error) {
	resourcesInfinispan, err := infinispan.CreateRequiredResources(instance, client)
	if err != nil {
		return
	}
