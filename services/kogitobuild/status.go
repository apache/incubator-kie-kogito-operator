...
func updateBuildsStatus(instance *appv1alpha1.KogitoBuild, client *client.Client) (changed bool, err error) {
	buildsStatus, err := openshift.BuildConfigC(client).GetBuildsStatusByLabel(
		...
		strings.Join([]string{
			...
			strings.Join([]string{LabelKeyBuildType, string(instance.Spec.Type)}, "="),
		}, ","))
	if err != nil {
		return false, err
	}
...
func handleConditionTransition(instance *appv1alpha1.KogitoBuild, client *client.Client) (changed bool, err error) {
	...
	err = kubernetes.ResourceC(client).ListWithNamespaceAndLabel(
		...
		map[string]string{
			...
			LabelKeyBuildType:     string(instance.Spec.Type)})
	if err != nil {
		return changed, err
	}
...
func HandleStatusChange(instance *appv1alpha1.KogitoBuild, client *client.Client, err *error) {
	needUpdate := false
	var statusErr error
	...
	if *err != nil {
		...
		if needUpdate, statusErr = handleConditionTransition(instance, client); statusErr != nil {
			err = &statusErr
			log.Errorf("Failed to update build status for %s: %v", instance.Name, err)
		}
	}
	trimConditions(instance)
	if needUpdate {
		if statusErr = update(instance, client); statusErr != nil {
			err = &statusErr
...
func update(instance *appv1alpha1.KogitoBuild, client *client.Client) error {
