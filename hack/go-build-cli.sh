...
if [ "$release" = "true" ]; then
  ...
  do
    ...
    packTemplateFiles
    CGO_ENABLED=0 GOOS="${i}" GOARCH="${arch}" go build -trimpath -ldflags=-buildid= -mod=mod -v -a -o build/_output/bin/kogito github.com/apache/incubator-kie-kogito-operator/cmd/kogito
    if [ $? -ne 0 ]; then
  ...
  packTemplateFiles
  CGO_ENABLED=0 go build -trimpath -ldflags=-buildid= -mod=mod -v -a -o build/_output/bin/kogito github.com/apache/incubator-kie-kogito-operator/cmd/kogito
  cleanTemplateFiles
fi
